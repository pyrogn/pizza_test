...
def test_menu(runner):
    ...
    assert len(result.output.split("\n")) == 1 + 1 + len(pizza_menu)  # 1 - header, 1 - pizza sizes
...
def test_pizza_order(is_delivery, pizza_class, runner):
    ...
    pizza_name = pizza_class.get_name()  # maybe add tests for upper and lower pizza names
    params_cli = ["order", pizza_name, "--delivery"] if is_delivery else ["order", pizza_name]
...
def test_pizza_wrong_size(pizza_class, runner):
    ...
    result = runner.invoke(cli, ["order", pizza_name, "--size", "definitely_unknown_size"])
    assert result.exit_code == 0
    assert len(result.output.split("\n")) == 2
